...
const (
	BotToken   = "1484216419:AAEqS13o3LnYmNZWCnhQlHbDESRp1i0m7TU"
	WebhookURL = "https://94ad53889aaa.ngrok.io"
)
...
func main() {
	...
	// получаем вс еобнавления из канала updates
	for update := range updates {
		if url, ok := rss[update.Message.Text]; ok {
			...
			bot.Send(tgbotapi.NewMessage(update.Message.Chat.ID, `there is only Habr feed available`))
		}
